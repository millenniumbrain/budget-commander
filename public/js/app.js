...
var overlay_1 = __webpack_require__(9);
...
var TransactionForm = (function (_super) {
    ...
    function TransactionForm(form) {
        ...
        _this.overlay = new overlay_1.default("newTransactionOverlay");
        _this.openTrigger = "addTransactionButton";
        _this.closeTrigger = "closeNewTransaction";
        _this.tranAccounts = document.getElementById("tranAccounts").getElementsByTagName("option");
        _this.tranTypeExpense = document.getElementById("tranTypeExpense");
        _this.tranTypeIncome = document.getElementById("tranTypeIncome");
        _this.tranButton = document.getElementById("tranButton");
        _this.setAccount = function (transaction) {
            // add accounts and select account associated with transaction
            for (var i = 0; i < _this.tranAccounts.length; i++) {
                if (_this.tranAccounts[i].innerHTML === transaction.accountName) {
                    _this.tranAccounts[i].setAttribute("selected", "true");
                }
            }
        };
        _this.setType = function (transaction) {
            switch (transaction.type) {
                case "income":
                    _this.tranTypeIncome.checked = true;
                    break;
                case "expense":
                    _this.tranTypeExpense.checked = true;
                    break;
                default:
                    break;
            }
        };
        _this.editTransaction = function () {
            var tranId = _this.$form.attr("data-id");
            _this.$form.on("submit", function (event) {
                event.preventDefault();
                $.ajax({
                    url: "/transactions/" + tranId,
                    type: "PUT",
                    data: JSON.stringify($(_this.$form).serializeArray())
                })
                    .fail(function (req) {
                })
                    .done(function () {
        ...
        _this.getAccounts = function () {
            $.get('/accounts', function (data) {
                var selectAccount = document.getElementById("tranAccounts");
                var accounts = selectAccount.getElementsByTagName('option');
                ...
                if (data.length > accounts.length) {
                    for (var _i = 0, data_1 = data; _i < data_1.length; _i++) {
                        ...
                        var accountName = document.createElement("option");
                ...
                .done(function () {
                _this.overlay.closeToggle(_this.closeTrigger, _this.clear);
            });
        };
        _this.clear = function () {
            ...
            _this.tranTypeExpense.checked = false;
            _this.tranTypeIncome.checked = false;
            for (var i = 0; i < _this.tranAccounts.length; i++) {
                // remove selection for form
                _this.tranAccounts[i].setAttribute("selected", "false");
            }
            _this.$form.off("submit");
        };
        return _this;
    }
    TransactionForm.prototype.init = function (transaction) {
    ...
    TransactionForm.prototype.setInputs = function (transaction) {
        var tranDate = document.getElementById("tranDate");
        tranDate.value = transaction.date;
        var tranAmount = document.getElementById("tranAmount");
        tranAmount.value = transaction.amount;
        var tranDesc = document.getElementById("tranDesc");
        tranDesc.value = transaction.desc;
        // add comma seperated tags to input field
        var tranTags = document.getElementById("tranTags");
    ...
    TransactionForm.prototype.submit = function () {
        _super.prototype.submit.call(this, "/transactions", this.overlay);
    };
...
var ItemList = (function () {
    function ItemList(buttonId, listId, titleId) {
        this.button = document.getElementById(buttonId);
        this.list = document.getElementById(listId);
        this.listTitle = document.getElementById(titleId);
    }
    ItemList.prototype.openList = function (url) {
        var _this = this;
        this.button.addEventListener("click", function () {
            // set slide animations depending on list state
            if (_this.list.getAttribute("class") === "slide-right") {
                _this.list.setAttribute("class", "");
                _this.button.setAttribute("class", "");
                _this.list.setAttribute("class", "slide-left");
            }
            else {
                $.get(url, function (data) {
                    _this.generateItems(data);
                })
                    .fail(function () {
                })
                    .done(function () {
                    _this.button.setAttribute("class", "active");
                    _this.list.setAttribute("class", "slide-right");
                });
            }
        }, false);
    };
    ItemList.prototype.createItem = function (callback) {
        var _this = this;
        var addItem = document.getElementById(this.addButtonId);
        var addItemContainer = document.getElementById(this.addButtonId).parentElement;
        ...
        addItem.addEventListener("click", function () {
            ...
            itemTotal.addEventListener("keydown", function (event) {
                itemTotal.setAttribute("contenteditable", "true");
                if (itemTotal.innerHTML.length > 0 && event.keyCode == 9) {
                    if (/([+-]?([0-9]*[.])?[0-9])/.test(itemTotal.innerHTML)) {
                        ...
                        _this.parseAmount(itemTotal, itemTotal.innerHTML);
                        itemTotal.blur();
                    }
                    else {
    ...
    ItemList.prototype.parseAmount = function (el, amount) {
        if (amount >= "0") {
    ...
    ItemList.prototype.editItems = function () {
    };
    ItemList.prototype.generateItems = function (items) {
        var _this = this;
        var nodes = this.list.getElementsByTagName('li');
...
var AccountList = (function (_super) {
    ...
    function AccountList(buttonId, listId, titleId) {
        var _this = _super.call(this, buttonId, listId, titleId) || this;
        _this.addButtonId = "addAccount";
        return _this;
    }
    AccountList.prototype.openList = function (url) {
        _super.prototype.openList.call(this, url);
    };
    AccountList.prototype.createItem = function () {
        _super.prototype.createItem.call(this);
    };
    AccountList.prototype.generateItems = function (items) {
        _super.prototype.generateItems.call(this, items);
    };
    return AccountList;
}(itemlist_1.default));
...
var TagList = (function (_super) {
    ...
    function TagList(buttonId, listId, titleId) {
        var _this = _super.call(this, buttonId, listId, titleId) || this;
        _this.addButtonId = "addTag";
        return _this;
    }
    TagList.prototype.openList = function (url) {
        _super.prototype.openList.call(this, url);
    };
    TagList.prototype.createItem = function () {
        _super.prototype.createItem.call(this);
    };
    TagList.prototype.generateItems = function (items) {
        _super.prototype.generateItems.call(this, items);
    };
    return TagList;
}(itemlist_1.default));
...
var Total = (function () {
    function Total() {
        var _this = this;
        this.parseAllTotals = function (data) {
            var networth = document.getElementById("networthTotal");
            var incomeTotal = document.getElementById("incomeTotal");
            var expenseTotal = document.getElementById("expenseTotal");
            _this.parseTotal(networth, data.networth);
            _this.parseTotal(incomeTotal, data.income);
            _this.parseTotal(expenseTotal, data.expense);
        };
    }
    Total.prototype.getAllTotals = function () {
...
Object.defineProperty(exports, "__esModule", { value: true });
var transactionform_1 = __webpack_require__(0);
var transaction_1 = __webpack_require__(12);
var TransactionTable = (function () {
    function TransactionTable() {
        var _this = this;
        this.tranTable = document.querySelector("#transactionActivity tbody");
        this.init = function () {
            _this.getTransactions();
            _this.tranTable.addEventListener("click", function (event) {
                // cast to HTMLElement to allow for HTMLElement methods
                var arrow = event.target;
                if (arrow.tagName === "i" || arrow.tagName === "I") {
                    _this.showControls(arrow);
                }
            }, false);
        };
        this.parseData = function (data) {
            var showTransactionsNum = document.getElementById("shownSize");
            var transacitonNum = document.getElementById("totalSize");
            for (var _i = 0, data_1 = data; _i < data_1.length; _i++) {
                var transaction = data_1[_i];
                ...
                _this.parseAmount(amountCell, transaction.type, transaction.amount);
                ...
                if (transaction.tags_data === undefined) {
                    ...
                    for (var _a = 0, _b = transaction.tags_data; _a < _b.length; _a++) {
                        var tag = _b[_a];
                ...
                _this.tranTable.appendChild(transactionRow);
            }
            $.get('/transactions?count=true', function (data) {
                transacitonNum.innerHTML = data;
            });
            showTransactionsNum.innerHTML = data.length;
        };
        this.parseAmount = function (el, type, amount) {
            switch (type) {
                case "income":
    ...
    TransactionTable.prototype.showControls = function (arrow) {
        var dataId = arrow.getAttribute("data-id");
        var row = document.getElementById(arrow.getAttribute("data-id"));
    ...
    TransactionTable.prototype.editTransaction = function (button, elementId) {
        var _this = this;
        button.addEventListener("click", function () {
            var transaction = _this.transactionRow(elementId);
            var editTransactionForm = new transactionform_1.default("#newTransaction");
            editTransactionForm.init(transaction);
        }, false);
    };
    TransactionTable.prototype.deleteTransaction = function (button, elementId) {
        var transactionRow = document.getElementById(elementId);
        button.addEventListener("click", function () {
            $.ajax({
    ...
    TransactionTable.prototype.getTransactions = function () {
    ...
    TransactionTable.prototype.transactionRow = function (elementId) {
        var transactionRow = document.getElementById(elementId);
        var columns = transactionRow.getElementsByTagName("td");
        ...
        var tags = document.querySelectorAll("tr[id='" + elementId + "'] span.table-tag");
...
var MessageBox = (function () {
    function MessageBox(parentId, msg) {
        var _this = this;
        this.close = function (delay) {
            var closeMessage = document.querySelector("#messageBox span");
            var messageBox = document.getElementById("messageBox");
            // allow message box to be removed when clicked
            closeMessage.addEventListener("click", function () {
                _this.parentElement.removeChild(messageBox);
            }, false);
            var remove = function () {
                try {
                    _this.parentElement.removeChild(messageBox);
        ...
        this.generateMessage = function (className) {
            ...
            messageBox.innerHTML = _this.msg;
            messageBox.appendChild(messageClose);
            _this.parentElement.insertBefore(messageBox, _this.parentElement.firstChild);
        };
        this.parentElement = document.getElementById(parentId);
        this.msg = msg;
    }
    MessageBox.prototype.showSuccess = function () {
        this.generateMessage("success");
    };
    MessageBox.prototype.showError = function () {
        this.generateMessage("error");
    };
    MessageBox.prototype.showWarning = function () {
...
var Overlay = (function () {
    function Overlay(overlay) {
        var _this = this;
        this.toggle = function () {
            if (_this.overlay.style.visibility === "visible") {
                _this.overlay.style.visibility = "hidden";
            }
            else {
                _this.overlay.style.visibility = "visible";
            }
        };
        this.overlay = document.getElementById(overlay);
    }
    Overlay.prototype.openToggle = function (openEl, callback) {
        var _this = this;
        // don't remember why I did this lol!
        if (openEl !== "" || false) {
            var openElement = document.getElementById(openEl);
    ...
    Overlay.prototype.closeToggle = function (el, callback) {
        var _this = this;
...
Object.defineProperty(exports, "__esModule", { value: true });
var messagebox_1 = __webpack_require__(8);
var Form = (function () {
    function Form(form) {
        this.$form = $(form); // eventual just use new FormData instead...
        this.form = document.getElementById(form.slice(1, form.length));
    }
    Form.prototype.submit = function (url, overlay) {
        var _this = this;
        this.$form.on("submit", function (event) {
            event.preventDefault();
            //loader.style.visibility = "visible";
            // get the form data right when it is sent
            var formData = JSON.stringify($(_this.$form).serializeArray());
            $.post(url, formData)
                .fail(function (req) {
                var error = new messagebox_1.default("dashboardContainer", req.responseJSON["msg"]);
                error.showError();
                error.close(5);
                overlay.toggle();
            })
                ...
                var success = new messagebox_1.default("dashboardContainer", response["msg"]);
                success.showSuccess();
                success.close(5);
                overlay.close();
            });
...
var taglist_1 = __webpack_require__(5);
var transactiontable_1 = __webpack_require__(7);
var total_1 = __webpack_require__(6);
var dropdown_1 = __webpack_require__(4);
var transactionform_1 = __webpack_require__(0);
var accountlist_1 = __webpack_require__(3);
$(document).ready(function () {
    var tagList = new taglist_1.default("tagListButton", "tagList", "tagListTitle");
    tagList.openList("/tags");
    tagList.createItem();
    var accountList = new accountlist_1.default("accountListButton", "accountList", "accountListTitle");
    ...
    var transactionForm = new transactionform_1.default("#newTransaction");
